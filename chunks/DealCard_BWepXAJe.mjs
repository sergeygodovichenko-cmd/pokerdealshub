import { c as createComponent, b as createAstro, a as renderTemplate, d as renderScript, u as unescapeHTML, e as addAttribute, m as maybeRenderHead } from './astro/server_C71Im3BI.mjs';
import 'kleur/colors';
import 'clsx';
import { g as getLangFromUrl, u as useTranslations } from './BaseLayout_DPLZ1pbk.mjs';
import { a as logoMap, r as renderStars, l as labl, b as badgeLabels, t as tooltips } from './dealHelpers_FzkXOya6.mjs';
...
const $$DealCard = createComponent(($$result, $$props, $$slots) => {
  ...
  }), addAttribute(`/${lang}/deal/${deal.slug}/`, "href"), deal.name[lang], deal.club && renderTemplate`<div> <span class="text-neutral-400">${labl[lang].club}</span> <span class="text-neutral-100 ml-2 font-medium">${deal.club}</span> </div>`, deal.union !== "N/A" && renderTemplate`<div> <span class="text-neutral-400">${labl[lang].union}</span> <span class="text-neutral-100 ml-2 font-medium">${deal.union}</span> </div>`, labl[lang].geog, typeof deal.geo === "string" ? deal.geo : deal.geo[lang], labl[lang].platform, deal.app, labl[lang].upTo, deal.rakeback, deal.description[lang], renderStars(deal.ratings.mtt), renderStars(deal.ratings.plo), renderStars(deal.ratings.nlh), addAttribute(`/${lang}/deal/${deal.slug}/`, "href"), t("deal.getButton"), unescapeHTML(JSON.stringify({
